...
ALTER PROCEDURE dbo.sp_ptg_QueueStandChangeEmail
(
...
BEGIN
    ...
    */
    ...
    IF NULLIF(LTRIM(RTRIM(@Ico)), N'') IS NOT NULL
    BEGIN
        DELETE c
        FROM #CandidateStands c
        WHERE NOT EXISTS
    ...
    BEGIN
        ...
        -- Aktualizace seznamu stojanů v emailu
        DELETE FROM dbo.EmailQueueStand
        WHERE IdEmailQueue = @ExistingEmailQueueId;

        INSERT INTO dbo.EmailQueueStand (IdEmailQueue, IdStand, ChangeDate, Ico)
        SELECT @ExistingEmailQueueId, cs.IdStand, cs.ChangeDate, NULLIF(LTRIM(RTRIM(@Ico)), N'')
        FROM #ChangedStands cs;
    ...
    BEGIN
        ...
        DECLARE @NewEmailQueueId bigint = SCOPE_IDENTITY();

        -- Zápis stojanů obsažených v emailu
        INSERT INTO dbo.EmailQueueStand (IdEmailQueue, IdStand, ChangeDate, Ico)
        SELECT @NewEmailQueueId, cs.IdStand, cs.ChangeDate, NULLIF(LTRIM(RTRIM(@Ico)), N'')
        FROM #ChangedStands cs;

        SELECT
            CAST(1 AS bit) AS EmailQueued,
            CONVERT(int, @NewEmailQueueId) AS EmailQueueId,
            @EffectiveFromDate AS FromDate,
            @Rows AS StandChangeCount,
